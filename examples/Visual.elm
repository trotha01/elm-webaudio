...
updateSlidersVisual ( w', h' ) isdown pos state =
  let
    ...
    updates =
      updateSliders ( ( w', h' ), isdown, pos ) { state | dimensions = ( w', h' ), scale = sliderScale, move = sliderMove }
  in
...
selectSlider ( w', h' ) ( x', y' ) state =
  let
    ...
    x =
      toFloat x' - w / 2.0 + state.move * 5.0
  ...
  case state.selected of
    Just ( _, slider ) ->
      let
        ...
        delta =
          (toFloat (snd state.lastPosition - snd pos)) / state.scale
