...
Elm.UserMedia = Elm.UserMedia || {};
Elm.UserMedia.make = function (_elm) {
   "use strict";
   _elm.UserMedia = _elm.UserMedia || {};
   if (_elm.UserMedia.values) return _elm.UserMedia.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Native$UserMedia = Elm.Native.UserMedia.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var requestUserMedia = F2(function (address,options) {
      return A2($Task.andThen,$Task.toMaybe($Native$UserMedia.getUserMedia(options)),$Signal.send(address));
   });
   var MediaStreamError = {};
   var MediaStream = F4(function (a,b,c,d) {    return {active: a,ended: b,label: c,id: d};});
   var Options = F2(function (a,b) {    return {audio: a,video: b};});
   return _elm.UserMedia.values = {_op: _op,Options: Options,MediaStream: MediaStream,MediaStreamError: MediaStreamError,requestUserMedia: requestUserMedia};
};
// setup
Elm.Native = Elm.Native || {};
Elm.Native.Graphics = Elm.Native.Graphics || {};
...
Elm.WebAudio.make = function (_elm) {
   ...
   if (_elm.WebAudio.values) return _elm.WebAudio.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $UserMedia = Elm.UserMedia.make(_elm);
   var _op = {};
   ...
   var Sine = {ctor: "Sine"};
   var createMediaStreamAudioSourceNode = $Native$WebAudio.createMediaStreamAudioSourceNode;
   ...
   var tapNode = F3(function (f,t,n) {    var _p0 = t(f(n));return n;});
   ...
   var AudioParam = function (a) {    return {ctor: "AudioParam",_0: a};};
   ...
   var OfflineAudioContext = F2(function (a,b) {    return {context: a,signal: b};});
   ...
   return _elm.WebAudio.values = {_op: _op
                                 ...
                                 ,createMediaStreamAudioSourceNode: createMediaStreamAudioSourceNode
                                 ,Sine: Sine
                                 ,Square: Square
                                 ,Sawtooth: Sawtooth
                                 ,Triangle: Triangle
...
Elm.Visual.make = function (_elm) {
   ...
   if (_elm.Visual.values) return _elm.Visual.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Color = Elm.Color.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   var _op = {};
   var renderAnalyser = F3(function (w,h,freqdata) {
      var barWidth = w / function (_p0) {    return $Basics.toFloat($List.length(_p0));}(freqdata);
      var draw = F2(function (idx,datum) {
         var barHeight = h * $Basics.toFloat(datum) / 255.0;
         return A2($Graphics$Collage.move,
         {ctor: "_Tuple2",_0: ($Basics.toFloat(idx) + 0.5) * barWidth,_1: (barHeight - h) / 2.0},
         A2($Graphics$Collage.filled,$Color.orange,A2($Graphics$Collage.rect,barWidth,barHeight)));
      });
      return A2($Graphics$Collage.groupTransform,A2($Transform2D.translation,0 - w / 2.0,h / -2.0),A2($List.indexedMap,draw,freqdata));
   });
   ...
   $Mouse.isDown,
   $Mouse.position);
   var playMusic = function (state) {    return _U.update(state,{loadTrack: true,playing: true});};
   var disableSelectedSlider = F2(function (pos,state) {
      return state.dragging ? _U.update(state,{lastPosition: pos,selected: $Maybe.Nothing,dragging: false,changes: true}) : _U.update(state,
      {lastPosition: pos,changes: false});
   });
   var scaleToFit = F3(function (_p1,desiredW,desiredH) {    var _p2 = _p1;return A2($Basics.min,desiredW / _p2.w,desiredH / _p2.h);});
   var controlState = {playing: false
                      ,loadTrack: false
                      ,url: A2($Graphics$Input$Field.Content,
                      "https://soundcloud.com/failemotions/gravity-instrumental",
                      A3($Graphics$Input$Field.Selection,0,0,$Graphics$Input$Field.Forward))
                      ,btnCnt: 0};
   var playButtonInput = $Signal.mailbox(controlState.playing);
   var playButtonCount = A3($Signal.foldp,F2(function (_p3,total) {    return total + 1;}),0,playButtonInput.signal);
   var urlFieldInput = $Signal.mailbox(controlState.url);
   var controlInput = A3($Signal.map2,F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};}),playButtonCount,urlFieldInput.signal);
   var renderControls = F2(function (w,state) {
      var btn = A2($Graphics$Input.button,A2($Signal.message,playButtonInput.address,$Basics.not(state.playing)),state.playing ? "Pause" : "Play");
      var url = A2($Graphics$Element.width,
      $Basics.round(w / 2) - $Graphics$Element.widthOf(btn),
      A4($Graphics$Input$Field.field,$Graphics$Input$Field.defaultStyle,$Signal.message(urlFieldInput.address),"SoundCloud Permalink URL",state.url));
      return $Graphics$Collage.toForm(A2($Graphics$Element.beside,btn,url));
   });
   var sliderValueClamp = function (_p4) {    return A2($Basics.max,-40.0,A2($Basics.min,40.0,_p4));};
   var updateSelectedSlider = F2(function (pos,state) {
      var _p5 = state.selected;
      if (_p5.ctor === "Just") {
            var _p7 = _p5._0._1;
            var delta = $Basics.toFloat($Basics.snd(state.lastPosition) - $Basics.snd(pos)) / state.scale;
            var currentVal = $WebAudio.getValue(_p7.gain);
            var newVal = sliderValueClamp(currentVal + delta);
            var _p6 = A2($WebAudio.setValue,newVal,_p7.gain);
            return _U.update(state,{lastPosition: pos,changes: true});
         } else {
            return _U.update(state,{lastPosition: pos,changes: false});
         }
   });
   var isSelectedSlider = F2(function (idx,state) {
      return A2($Maybe.withDefault,false,A2($Maybe.map,function (_p8) {    var _p9 = _p8;return _U.eq(_p9._0,idx);},state.selected));
   });
   var slidersState = {dimensions: {ctor: "_Tuple2",_0: 0,_1: 0}
                      ,dragging: false
                      ,lastPosition: {ctor: "_Tuple2",_0: 0,_1: 0}
                      ,selected: $Maybe.Nothing
                      ,scale: 1.0
                      ,move: 0.0
                      ,changes: false};
   var sliderSize = {w: 20.0,h: 100.0};
   var handleHitTest = F4(function (x,y,s,f) {
      ...
      return _U.cmp(handleLeft,x) < 1 && (_U.cmp(x,handleRight) < 1 && (_U.cmp(handleBottom,y) < 1 && _U.cmp(y,handleTop) < 1));
   });
   var renderSlider = F2(function (val,selected) {
      var handleColor = selected ? $Color.lightRed : $Color.darkRed;
      return _U.list([A2($Graphics$Collage.filled,$Color.black,A2($Graphics$Collage.rect,sliderSize.w / 4.0,sliderSize.h - sliderSize.w))
                     ,A2($Graphics$Collage.moveY,val,A2($Graphics$Collage.filled,handleColor,A2($Graphics$Collage.rect,sliderSize.w,sliderSize.w)))]);
   });
   var analyser = A4($WebAudio.connectNodes,$WebAudio.getDestinationNode($WebAudio.DefaultContext),0,0,$WebAudio.createAnalyserNode($WebAudio.DefaultContext));
   var doOrDie = F3(function (f,lst,err) {
      var x = f(lst);
      var _p10 = x;
      if (_p10.ctor === "Just") {
            return _p10._0;
         } else {
            return _U.crashCase("Visual",{start: {line: 23,column: 5},end: {line: 28,column: 18}},_p10)(err);
         }
   });
   ...
   var filters = function () {
      var makeFilter = function (f) {
         return A3($WebAudio.tapNode,
         function (_) {
            return _.frequency;
         },
         function (freq) {
            return A2($WebAudio.setValue,f,freq);
         },
         A2($WebAudio.setFilterType,$WebAudio.Peaking,$WebAudio.createBiquadFilterNode($WebAudio.DefaultContext)));
      };
      var frequencies = _U.list([31.25,62.5,125,250,500,1000,2000,4000,8000,16000]);
      var rlst = A2($List.map,makeFilter,$List.reverse(frequencies));
      var end = A4($WebAudio.connectNodes,analyser,0,0,A2($WebAudio.setFilterType,$WebAudio.HighShelf,A2(headOrDie,rlst,"no filters")));
      var lst = $List.reverse(A3($List.scanl,F2(function (c,p) {    return A4($WebAudio.connectNodes,p,0,0,c);}),end,A2(tailOrDie,rlst,"no filters")));
      var _p12 = lst;
      if (_p12.ctor === "::") {
            return A2($List._op["::"],A2($WebAudio.setFilterType,$WebAudio.LowShelf,_p12._0),_p12._1);
         } else {
            return _U.list([]);
         }
   }();
   var mediaStream = A4($WebAudio.connectNodes,
   A2(headOrDie,filters,"no filters"),
   0,
   0,
   A2($WebAudio.setMediaElementIsLooping,true,$WebAudio.createHiddenMediaElementAudioSourceNode($WebAudio.DefaultContext)));
   var updateTrack = function (_p13) {
      return $WebAudio.playMediaElement(A2($Maybe.withDefault,
      mediaStream,
      A2($Maybe.map,function (url) {    return A2($WebAudio.setMediaElementSource,url,mediaStream);},_p13)));
   };
   var mainMediaStream = A2($Signal.map,updateTrack,soundUrl);
   var pauseMusic = function (state) {    var _p14 = $WebAudio.pauseMediaElement(mediaStream);return _U.update(state,{playing: false,loadTrack: false});};
   var toggleMusic = function (state) {    return state.playing ? pauseMusic(state) : playMusic(state);};
   var updateControls = F2(function (_p15,state) {
      var _p16 = _p15;
      var _p18 = _p16._1;
      var _p17 = _p16._0;
      return !_U.eq(_p17,state.btnCnt) ? toggleMusic(_U.update(state,{btnCnt: _p17,url: _p18})) : _U.update(state,{url: _p18,loadTrack: false});
   });
   var mainControls = A3($Signal.foldp,updateControls,controlState,controlInput);
   ...
   function (_p19) {
      var _p20 = _p19;
      return _p20.url.string;
   },
   A3($Signal.filter,function (_p21) {    var _p22 = _p21;return _p22.loadTrack;},controlState,mainControls)));
   var selectSlider = F3(function (_p24,_p23,state) {
      var _p25 = _p24;
      var _p26 = _p23;
      var _p31 = _p26._1;
      var _p30 = _p26._0;
      var lst = A2($List.indexedMap,F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};}),filters);
      var _p27 = {ctor: "_Tuple2",_0: $Basics.toFloat(_p25._0),_1: $Basics.toFloat(_p25._1)};
      var w = _p27._0;
      var h = _p27._1;
      var x = $Basics.toFloat(_p30) - w / 2.0 + state.move * 5.0;
      var y = h / 4.0 - $Basics.toFloat(_p31);
      var filtered = A2($List.filter,
      function (_p28) {
         var _p29 = _p28;
         return A4(handleHitTest,x - ($Basics.toFloat(_p29._0) + 0.5) * state.move,y,state.scale,_p29._1);
      },
      lst);
      var selected = $List.head(filtered);
      return A2(updateSelectedSlider,{ctor: "_Tuple2",_0: _p30,_1: _p31},_U.update(state,{selected: selected,dragging: true}));
   });
   var updateSliders = F2(function (_p32,state) {
      var _p33 = _p32;
      var _p36 = _p33._2;
      var _p35 = _p33._1;
      var _p34 = _p33._0;
      return !_U.eq(_p34,state.dimensions) ? A4(updateSlidersVisual,_p34,_p35,_p36,state) : _p35 ? state.dragging ? A2(updateSelectedSlider,
      _p36,
      state) : A3(selectSlider,_p34,_p36,state) : A2(disableSelectedSlider,_p36,state);
   });
   var updateSlidersVisual = F4(function (_p37,isdown,pos,state) {
      var _p38 = _p37;
      var _p41 = _p38._0;
      var _p40 = _p38._1;
      var _p39 = {ctor: "_Tuple2",_0: $Basics.toFloat(_p41),_1: $Basics.toFloat(_p40)};
      var w = _p39._0;
      var h = _p39._1;
      var sliderScale = A3(scaleToFit,sliderSize,w / 10.0 * 0.9,h / 2.0 * 0.9);
      var sliderMove = sliderScale * sliderSize.w / 0.9;
      var updates = A2(updateSliders,
      {ctor: "_Tuple3",_0: {ctor: "_Tuple2",_0: _p41,_1: _p40},_1: isdown,_2: pos},
      _U.update(state,{dimensions: {ctor: "_Tuple2",_0: _p41,_1: _p40},scale: sliderScale,move: sliderMove}));
      return _U.update(updates,{changes: true});
   });
   var mainSliders = A3($Signal.filter,function (m) {    return m.changes;},slidersState,A3($Signal.foldp,updateSliders,slidersState,slidersInput));
   var renderSliders = F3(function (w,h,state) {
      var slider = F2(function (idx,filter) {
         var selected = A2(isSelectedSlider,idx,state);
         var val = $WebAudio.getValue(filter.gain);
         var x = ($Basics.toFloat(idx) + 0.5) * state.move;
         return A2($Graphics$Collage.groupTransform,A6($Transform2D.matrix,state.scale,0,0,state.scale,x,0),A2(renderSlider,val,selected));
      });
      return A2($Graphics$Collage.groupTransform,A2($Transform2D.translation,0 - state.move * 5.0,h / 2.0),A2($List.indexedMap,slider,filters));
   });
   var render = F5(function (_p43,sliderState,controlState,freqdata,_p42) {
      var _p44 = _p43;
      var _p47 = _p44._0;
      var _p46 = _p44._1;
      var _p45 = {ctor: "_Tuple2",_0: $Basics.toFloat(_p47),_1: $Basics.toFloat(_p46)};
      var w = _p45._0;
      var h = _p45._1;
      var halfw = w / 2.0;
      var halfh = h / 2.0;
      ...
      _p47,
      _p46,
      _U.list([A2($Graphics$Collage.moveY,quarterh,A2($Graphics$Collage.filled,A3($Color.rgb,34,34,34),A2($Graphics$Collage.rect,w,halfh)))
              ,A2($Graphics$Collage.moveY,0 - quarterh,A2($Graphics$Collage.filled,$Color.black,A2($Graphics$Collage.rect,w,halfh)))
              ,A3(renderSliders,w,halfh,sliderState)
              ,A3(renderAnalyser,w,halfh,freqdata)
              ,A2(renderControls,w,controlState)]));
   });
   ...
   mainSliders,
   mainControls,
   A2($Signal.map,function (_p48) {    return $WebAudio.getByteFrequencyData(analyser);},$Time.every(50.0)),
   mainMediaStream);
   return _elm.Visual.values = {_op: _op
                               ,doOrDie: doOrDie
                               ,headOrDie: headOrDie
                               ,tailOrDie: tailOrDie
                               ,analyser: analyser
